...
def MSVAGE_GAN():
    ...
    gan_gen_1 = MLP_G(ninput=args.z_size, noutput=args.hidden1_dim, dropout=0.1).cuda()
    ...
    gan_disc1 = MLP_D(ninput=96, noutput=2, dropout=0.1).cuda()
    ...
    for epoch in range(301):
        ...
        for sub in range(len(label_n)):
            ...
            #Norm=nn.BatchNorm1d(62).cuda()
            #feature=Norm(features[sub])
            #sns.heatmap(features[sub].cpu().detach().numpy(), cmap='viridis')
